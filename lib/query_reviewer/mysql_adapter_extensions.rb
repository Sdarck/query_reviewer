...
module QueryReviewer
  module MysqlAdapterExtensions
    def self.included(base)
      base.alias_method :select, :review
      base.alias_method :update, :review
      base.alias_method :insert, :review
      base.alias_method :delete, :review
    end
    ...
    def select_with_review(sql, *args)
      ...
      if @logger && query_reviewer_enabled? && sql =~ /^select/i
        ...
        if QueryReviewer::CONFIGURATION['production_data']
          use_profiling &&= (t2 - t1) >= QueryReviewer::CONFIGURATION['warn_duration_threshold'].to_f / 2.0
        end
