...
module QueryReviewer
  ...
  class SqlQueryCollection
    ...
    def total_duration
      queries.collect(&:durations).flatten.sum
    end
    ...
    def warnings
      queries.collect(&:warnings).flatten.sort { |a, b| b.severity <=> a.severity }
    end

    def without_warnings
      queries.reject(&:has_warnings?).sort { |a, b| b.duration <=> a.duration }
    end
    ...
    def max_severity
      if warnings.empty? && collection_warnings.empty?
        0
      else
        [
          warnings.empty? ? 0 : warnings.collect(&:severity).flatten.max, collection_warnings.empty? ? 0 : collection_warnings.collect(&:severity).flatten.max
        ].max
      end
    end
