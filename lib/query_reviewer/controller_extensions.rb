  ...
  module ControllerExtensions
    ...
    def self.included(base)
      if QueryReviewer::CONFIGURATION['inject_view']
        ...
        base.alias_method(alias_name, :query_review)
      end
      base.alias_method :process, :query_review
    ...
    def add_query_output_to_view(total_time)
      if request.xhr?
        if cookies['query_review_enabled']
          ...
          elsif response.content_type&.include?('text/javascript')
            response.body += ";\n#{query_review_output(true, total_time)}"
          end
        end
      elsif !response.instance_eval { @body.is_a?(File) || @body.is_a?(Enumerator) } &&
            response.body.is_a?(String) && response.body.match(%r{</body>}i)
        idx = (response.body =~ %r{</body>}i)
    ...
    alias process_action_with_query_review perform_action_with_query_review
